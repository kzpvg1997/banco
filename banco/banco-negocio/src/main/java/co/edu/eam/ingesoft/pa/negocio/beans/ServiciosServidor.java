...
/**
 * @author TOSHIBAP55W
 * Calse encargada de los servicios del servidor 
 *
 */
@LocalBean
@Stateless
public class ServiciosServidor implements Serializable{
	...
	public void transferirMonto (String idBanco,String numeroCuenta, double monto){
		...
		String endpoint="http://104.197.238.134:8080/interbancario/InterbancarioWS";
	     ...
	     if (resp.getCodigo().equals("0000")){//Si la trasnferencia es exitosa
	    	 ...
	    	 if(resp.getCodigo().equals("0002")){// si la cuenta aun no esta asociada
	    		 System.out.println("la cuenta aun no esta asociada");
		    	 resp.getMensaje(); 
	    	 }else{
	    		 if(resp.getCodigo().equals("0002")){// si la Cuenta no existe en el banco de destino
	...
	@TransactionAttribute(TransactionAttributeType.REQUIRED)
	public void asociarCuenta (String idbanco,String tipodoc,String numerodoc,String nombreAsociado,String numerocuenta){
	    ...
	    RespuestaServicio resp= servicio.registrarCuentaAsociada(idbanco, tipo, numerodoc, nombreAsociado, numerocuenta);
	    ...
	    if(cu!=null){
	    	...
	    	if(resp.getCodigo().equals("0010")){//si el banco no existe
	    		...
	    		if(resp.getCodigo().equals("0001")){//si la cuenta esta pendiente
	    			resp.getMensaje();
	    			...
	    			if(resp.getCodigo().equals("0000")){//si la cuenta esta asociada
	    				CuentaAsociados cuen = asoEJB.buscarCuentaAsociado(numerocuenta);
	    				cuen.setEstado(resp.getMensaje());
	    				em.merge(cuen);
